...
def test_course_inserts_prerequisite_with_graph_exam_context_and_unlocks_dependent():
    ...
    assert course[1].status == "BLOCKED_BY_PREREQUISITE"

    rebuilt = build_course(plan, {"algorithms.tracing": {"mastered": True}})
    assert [item.skill_id for item in rebuilt] == ["algorithms.tracing", "algorithms.recursion"]
    assert [item.status for item in rebuilt] == ["MASTERED", "READY"]
