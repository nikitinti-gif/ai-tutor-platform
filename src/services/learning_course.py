...
def build_course(plan: Iterable[dict], states: dict | None = None) -> list[CourseItem]:
    ...
    for skill_id in requested:
        # Task 14's reference path already contains its internal foundation
        # checkpoints, so it stays a single restart-compatible course item.
        path = (
            [skill_id]
            if skill_id == "number_systems.large_number_digits"
            else prerequisite_path(skill_id, skill_map)
        )
    ...
    for skill_id in ordered:
        ...
        if mastered:
            ...
            prerequisite_name = get_skill(unmet[0], skill_map)["name"]
        ...
        result.append(CourseItem(
            ...
            reason=("Пробел подтверждён диагностическими evidence" if rows else
                    "Сначала освоить prerequisite"),
