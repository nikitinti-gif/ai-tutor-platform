...
TASK27_REMEDIATION = {
    ...
    "programming.medoid_minimum": {
        "explanation": "В №27 центр кластера выбирают среди исходных точек по минимальной сумме расстояний до остальных точек этого кластера.",
        "example": "Если для кандидатов A, B, C суммы расстояний равны 12, 9 и 15, центр по условию — B, потому что 9 — минимум.",
        "hint": "Выбери букву с наименьшей суммой.",
        "control_prompt": "Контроль: суммы расстояний для точек A=14, B=11, C=17. Какая точка будет центром кластера? Ответь A, B или C.",
        "control_answers": ("B", "Б", "11"),
        "retest_prompt": "Перенос: суммы расстояний для точек A=21, B=16, C=19. Какая точка будет центром кластера? Ответь A, B или C.",
        "retest_answers": ("B", "Б", "16"),
        "verification_prompt": "Независимая проверка: суммы расстояний для точек A=18, B=23, C=15. Какая точка будет центром кластера? Ответь A, B или C.",
...
TUTOR_PILOT_TASKS = {
    ...
    27: {
        "title": "Кластеризация и центр кластера",
        "teacher_intro": (
            "В актуальных заданиях №27 центр кластера задаётся прямо в условии: это одна из исходных точек, "
        ...
        "worked_example": (
            "Пример: для точек (0,0), (0,2), (0,4) центр по условию ЕГЭ — (0,2): "
        ...
        "statement": (
            ...
            "A = {(0,0), (0,2), (0,4)}\n"
            "B = {(10,10), (12,10), (14,10)}\n\n"
            "Найди центр каждого кластера по этому определению. Затем сложи все четыре координаты двух найденных центров."
        ),
        "exam_bridge": (
...
TUTOR_PILOT_TRANSFER_TASKS = {
    ...
    27: {
        "statement": (
            ...
            "B={(20,20),(22,20),(24,20)}. Центром кластера считается одна из его точек с минимальной "
