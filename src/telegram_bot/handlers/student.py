...
async def start_personal_learning(message: Message, state: FSMContext) -> None:
    ...
    for course_item in course:
        ...
        if course_item.status == "READY" and skill_id in executable_skill_ids() and attempt is not None:
    ...
    # Preserve the task-specific №5/№27 remediation routes after the global
    # course has had a chance to select any READY module.
    for item in plan:
